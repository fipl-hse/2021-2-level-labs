...
в котором хранятся зарегистрированные языкове профили.
...
из себя словарь, с ключом - меткой языка, а значением - соответсвующим языковым профилем.
...
это может занять много времени и вычислительных ресурсов. Созданный единажды языковой
...
строковый формат, с которым работает `.json`, иначе вам не удасться сохранить профиль. То есть условная биграмма `(2, 3)` должна
...
- **Для корректного открытия поля `freq` необхоимо разделить все представленные N-граммы на
группы в соответсвии с размерностью N-грамм, а затем создать под каждую размерность N-грамм
...
будут учавствовать в сравнении, а также `trie_level` - рамзерность N-грамм (например, биграммы).

Метод возвращает вероятность того, что неизвестный языковой профиль принадлежит известному языковому профилю.

Метод считает принадленость N-грамм по заданному уровню `trie_level`. 
...
профиля к общей лог-вероятности и так далее для топ `k` частотных N-грамм, которые присутсвуют в 
`unknown_profile`.
...
и параметр `trie_levels`, который обозначает размерности N-грамм, учавствующих в сравнении. 
...
Вам дано предложение на экзотическом языке:
```
