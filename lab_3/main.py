...
def encode_corpus(storage: LetterStorage, corpus: tuple) -> tuple:
    ...
    for sentence in corpus:
        ...
        encoded__corpus_list.append(tuple(sentence_list))
    return tuple(encoded__corpus_list)
...
def decode_corpus(storage: LetterStorage, corpus: tuple) -> tuple:
    ...
    for sentence in corpus:
        ...
        for word in sentence:
            ...
            sentence_list.append(tuple(word_list))
        decoded__corpus_list.append(tuple(sentence_list))
    return tuple(decoded__corpus_list)
...
class LanguageProfile:
    ...
    def create_from_tokens(self, encoded_corpus: tuple, ngram_sizes: tuple) -> int:
        ...
        for ngram_size in ngram_sizes:
            ngram_trie = NGramTrie(ngram_size, self.storage)
    ...
    def get_top_k_n_grams(self, k: int, trie_level: int) -> tuple:
        ...
        for ngram_trie in self.tries:
            if ngram_trie.size == trie_level:
                frequency = ngram_trie.n_gram_frequencies
                top_k_ngrams = sorted(frequency, key=frequency.get, reverse=True)[:k]
                return tuple(top_k_ngrams)
        return ()
...
def calculate_distance(unknown_profile: LanguageProfile, known_profile: LanguageProfile,
    ...
    for ngram in unknown_top_ngrams:
        if ngram in known_top_ngrams:
            distance += abs(unknown_top_ngrams.index(ngram) - known_top_ngrams.index(ngram))
        else:
            distance += len(known_top_ngrams)
    return distance
