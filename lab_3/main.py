...
def tokenize_by_sentence(text: str) -> tuple:
    ...
    for sentence in sentences:
        ...
        for word in sentence:
            for umlaut, change in umlauts.items():
                word = word.replace(umlaut, change)
        ...
        for word in clear_sent:
            ...
            if letters:
                letters.insert(0, '_')
                letters.append('_')
                new_sent.append(tuple(letters))
            if not new_sent:
                return ()
        end_new_text.append(tuple(new_sent))
    return tuple(end_new_text)
...
def encode_corpus(storage: LetterStorage, corpus: tuple) -> tuple:
    ...
    for sentence in corpus:
        ...
        for word in sentence:
            ...
            encoded_sent.append(tuple(id_word))
...
def decode_corpus(storage: LetterStorage, corpus: tuple) -> tuple:
    ...
    for encoded_sent in corpus:
        ...
        decoded_corpus.append(tuple(sentence))
...
def calculate_distance(unknwon_profile: LanguageProfile, known_profile: LanguageProfile,
    ...
    if not isinstance(unknwon_profile, LanguageProfile) \
        ...
        return -1
    top_unknown_ngrams = unknwon_profile.get_top_k_n_grams(k, trie_level)
    top_known_ngrams = known_profile.get_top_k_n_grams(k, trie_level)
    distance = 0
    for ngram in top_unknown_ngrams:
        if ngram in top_known_ngrams:
            distance += abs(top_known_ngrams.index(ngram) - top_unknown_ngrams.index(ngram))
        else:
            distance += len(top_known_ngrams)
    return distance
