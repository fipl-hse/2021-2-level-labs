...
def tokenize_by_sentence(text: str) -> tuple:
    ...
    umlauts_to_replace = {"ö": "oe", "ü": "ue", "ä": "ae", "ß": "ss"}
    list_text = []
    cleaned_sentence = ''
    for sentence in text:
        for symbol in sentence:
            if symbol in umlauts_to_replace:
                cleaned_sentence += umlauts_to_replace[symbol]
            elif symbol.isalpha() or symbol.isspace():
                cleaned_sentence += symbol
        list_text.append(cleaned_sentence.split())
        cleaned_sentence = ''
    ...
    for sentence in list_text:
        ...
        if list_sentence:
            prepared_text.append(tuple(list_sentence))
...
class LetterStorage:
    ...
    def get_id_by_letter(self, letter: str) -> int:
        ...
        :return: an id
        """
        if not (isinstance(letter, str) and letter in self.storage):
            return -1
    ...
    def get_letter_by_id(self, letter_id: int) -> str or int:
        ...
        if not (isinstance(letter_id, int)
                and letter_id in self.storage.values()):
            return -1
...
def decode_corpus(storage: LetterStorage, corpus: tuple) -> tuple:
    ...
    for sentence in corpus:
        ...
        for token in sentence:
            encoded_token = ()
            for letter in token:
                encoded_token += (storage.get_letter_by_id(letter),)
            encoded_sentence += (encoded_token,)
        encoded_corpus += (encoded_sentence,)
    return encoded_corpus
